...
2. GitHub Pages 기본 저장소
   ...
   >>package.json파일 가장 상위에 ```"homepage": "https://yjh031201.github.io``` 추가   
   >>"scripts"에 ```"predeploy": "npm run build", "deploy": "gh-pages -d dist"``` 추가    
   >> 터미널에서 npm run deploy를 통해 원격 배포   
   >> https://yjh031201.github.io/ 주소로 접속가능
# 2026-05-27
...
6. 렌더링 과정의 3단계
   ...
   >### 2단계 : React 컴포넌트 렌더링
   >>재귀적 단계: 업데이트된 컴포넌트가 다른 컴포넌트를 반환하면 React는 다음으로 해당   
   >>컴포넌트를 렌더링하고 해당 컴포넌트도 컴포넌트를 반환하면 반환된  컴포넌트를 다음에 렌더링하는 방식   
   >>중첩된 컴포넌트가 더 이상 없고 React가 화면에 표시되어야 하는 내용을 정확히 알 때까지 이 단계는 계속됨   
   >### 3단계 : React가 DOM에 변경사항을 커밋
   >>React는 컴포넌트를 렌더링한 후에 DOM을 수정함   
   >>초기 렌더링의 경우는 appendChild() DOM API를 사용해서, 생산한 모든 DOM노드를 화면에 표시함   
   >>리렌더링의 경우는 최신 렌더링의 출력과 일치하도록 DOM을 변경하기 위해 필요한 최소한의 작업을 적용함
7. 스냅샷처럼 동작하는 State
   >state 변수를 set함수로 업데이트해도 이미 가지고 있는 state변수는 변경되지 않고, 대신 리렌더링이 촉발(트리거)된다.   
   >컴포넌트의 state에서는 set함수의 호출이 트리거로 작용하여 렌더링이 이루어진다.
   >#### 스냅샷(snapshot)
   >>스냅샷은 
# 2026-05-20
### 이미지를 배열로 저장
